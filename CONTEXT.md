...
| `sortItems` / **`itemsOn`** | **손으로 정한 `order` 가 제일 먼저**, 없으면 뒤로(`ordOf`). 그다음 하루종일→시간→**제목**순 정렬(`localeCompare` 를 안 쓴다 — selftest 가 `lang='en'` 에서 정렬 불변을 본다). **`itemsOn` 이 `state.filter` 를 직접 읽는다** — 화면에 할 일을 꺼내는 통로가 이 함수뿐이라(월·주·일·하단 목록·내보내기 3곳) 여기 한 줄로 전부 따라온다. 인자로 넘기면 6곳 중 하나만 빠뜨려도 그 화면에서 필터가 **조용히** 안 먹는다(내보내기가 제일 위험). 순수함을 잃는 대가로 그 실수를 없앴다. **종류 필터(`state.kind`)도 같은 문을 지난다** — 날짜와 무관한 조건은 `passes()` 로 빼서 막대 층(`weekEventBars`)이 **같은 함수**를 지나게 했다. `kindFilter()` 가 일간·연간에서만 값을 무시한다(지우지 않는다 — 지우면 월간으로 돌아왔을 때 고른 것이 풀린다). **`cellItems`** 는 여기서 일정을 뺀 것 = 월·주 격자의 **칸**에 들어가는 것.<br>★ `order` 를 여기 넣은 이유는 "달력 칸까지 반영" 이다(사용자 요청) — 이 함수가 통로라 한 줄이면 월 칸·주 열·아래 목록·내보내기가 전부 따라온다. **같이 따라오는 것이 하나 더 있다**: `weekEventBars` 가 이 순서로 막대 **층**을 배정하므로 일정 순서를 바꾸면 달력의 막대가 위아래로 움직인다.<br>★ `order` 는 **항목마다 하나**다(날짜별이 아니다). 반복·여러 날 항목은 여러 날에 나타나지만 문서는 하나라, 한 날에서 바꾼 순서가 그 항목이 나오는 **모든 날**에 적용된다. 날짜별로 두려면 항목마다 날짜→번호 맵을 들어야 해서 문서가 계속 자란다 — 개인용 앱에는 과하다고 보고 안 했다(사용자와 합의).<br>★ **`eventsFirst`** 는 이 정렬 **바깥**에 있다 — 화면 아래 목록과 일간 이미지 두 곳에서만 일정을 앞으로 모은다. 종류가 바뀌는 자리에 굵은 선(화면 3px · 이미지 7px)을 **하나만** 긋기 위해서다. 시각 순 그대로면 09시 할 일이 14시 일정 위로 올라와 두 종류가 섞여서 그을 자리가 안 생긴다. ⚠️ `sortItems`·`itemsOn` 안으로 옮기지 말 것 — 월·주 칸의 알약과 막대 층이 같은 통로를 써서 격자가 통째로 흔들린다. 이미지 쪽 굵은 선은 **높이를 안 먹는다**(줄 사이에 이미 긋던 실선을 두껍게만 그린다) — `exDayLayout` 이 이미지 높이를 정하고 selftest 가 그 값을 붙잡고 있다 |
...
| **`exportModel` / `exAttachDetail`** | 그리기 전 데이터 모델. **`includeMemo` 가 false 면 `memo`·`cat` 키를 안 만든다**(기본이 false 다) |
| **`subLine` / `syncMemoBtns`** | 아래 목록의 **둘째 줄** = [카테고리(그 색)] · [메모] [펼침]. 카테고리가 없는 항목엔 이름을 안 붙인다(폴백이 '없음' 이라 잡음이 된다).<br>★ 접히면 메모의 **첫 줄만**(내용 있는 첫 줄), 펼치면 `white-space:pre-wrap` 으로 **줄바꿈까지 그대로** 나온다. pre-wrap 이 없으면 HTML 이 `\n` 을 공백 하나로 뭉개 여러 줄 메모가 한 문단처럼 이어진다.<br>★ 펼침 버튼이 뜨는 조건은 **둘**이다 — 뒷줄이 있거나(`data-memomore`, 렌더 때 안다) 첫 줄이 가로로 잘렸거나. 잘렸는지는 CSS 로 못 묻고 `scrollWidth > clientWidth` 로 **그린 뒤에** 재야 해서, `render()` 끝(innerHTML **뒤**)에서 `syncMemoBtns()` 가 켜고 끈다. markup 은 늘 `hidden` 으로 낸다 — 반대면 짧은 메모에서도 버튼이 한 번 번쩍인다. ⚠️ 잘림만 보던 옛 규칙으로 되돌리지 말 것 — 첫 줄이 짧고 뒷줄이 있는 메모는 꺼낼 방법이 사라진다.<br>★ 펼친 표시는 **`state.memoOpen`** 에 있다. `render()` 가 `#app` 을 통째로 다시 만들어서 요소에 두면 다음 렌더에 조용히 접힌다. 저장되지 않는 화면 전용 값이다.<br>★ 클릭 위임에서 `[data-memo]` 는 **`[data-open]` 앞**이다. 버튼이 그 안에 들어 있어서, 뒤에 두면 펼치려 누를 때마다 편집기가 열린다.<br>★ 메모 칸은 `flex:1;min-width:0` 이라야 `.trunc` 가 먹는다 — 없으면 말줄임 대신 줄이 가로로 밀린다 |
...
| **`dragPaint` / `dragDrop` / `dragEnd`** (+ `catDragCfg` · `itemDragCfg` · `DRAG_HOLD_MS` 400 · `DRAG_SLOP` 10 · `DRAG_LIFT` 1.03) | **순서 끌기 — 손잡이(`data-draghandle`)는 누르는 즉시, 줄 본문은 길게 눌러야 잡힌다.** 카테고리 관리 목록과 **아래 할 일 목록이 같은 엔진**을 쓴다 — 다른 것은 셋뿐이라 그 셋만 인자로 받는다(`sel` 어떤 줄인가 · `lo`·`hi` 넘어갈 수 없는 경계 · `drop` 놓았을 때 무엇을 하는가). ⚠️ 두 벌로 복사하지 말 것.<br>★ 아래 목록은 **제 무리 안에서만** 끌린다(`lo`·`hi` 가 `data-itemkind` 가 이어지는 구간이다) — 일정/할 일 굵은 선을 넘으면 "선 위 = 일정" 이 거짓이 된다. 무리에 혼자면 손잡이를 아예 안 그린다.<br>★ 끄는 차례는 **그려진 DOM 에서** 읽는다(`data-itemdrag`) — `selShown` 은 어디에도 안 남고, 다시 계산하면 정렬·묶기 규칙을 두 번 적는 셈이다.<br>★ `commitItemOrder` 는 `saveTodo(id, {order})` **한 필드만** 보낸다. `saveTodo` 가 merge 라 나머지가 안 날아간다 — `saveCat`(setDoc 통째 교체)과 정반대다.<br>★ `todos` 규칙에는 필드 제약이 없어서 `order` 를 새로 실어도 **규칙 배포가 필요 없다**(categories·goals 와 다르다). 위임은 `app` 의 `pointerdown`·`pointermove`·`touchmove` 와 **창 단위**의 `pointerup`·`pointercancel` 이다(마우스를 `#app` 밖에서 떼면 app 리스너로 안 온다).<br>★ 끄는 동안 **`render()` 를 한 번도 안 부른다** — `#app` 을 갈아엎으면 손가락이 잡은 줄이 사라진다(로그인 화면 키보드와 같은 원리). `syncSheet` 처럼 DOM 을 직접 민다.<br>★ **잡힌 줄은 `translate`/`scale` 개별 속성, 비켜 주는 줄은 `transform`.** 가른 이유는 transition 이다 — 뜨는 것(`scale`·그림자 .16s)만 부드럽고 **이동에는 transition 이 없어야** 줄이 손에서 안 뒤처진다. 잡힌 줄의 이동을 `transform` 으로 되돌리면 그 구분이 무너진다. `scale` 은 1.03 이 천장 — 레이아웃은 안 건드려도 **스크롤 넓이에는 잡혀서** 더 키우면 시트에 가로 스크롤바가 생긴다.<br>★ 한 줄 높이는 **이웃한 두 줄의 간격을 잰다.** 첫 줄만 위 테두리(.5px)가 없어서 높이가 다르다.<br>★ 줄 본문은 **400ms 안에 움직이면 스크롤로 본다** — 안 그러면 시트를 굴리려는 손짓이 전부 순서 바꾸기가 된다. 손잡이는 안 기다리는 대신 `touch-action:none` 이 필요하다(없으면 브라우저가 스크롤을 먼저 채 간다). 손잡이의 위아래 `padding:10px` 과 **같은 크기의 음수 margin** 이 짝 — 누르는 띠만 36px 로 넓히고 줄 높이는 안 움직인다. 잡힌 뒤에는 `touchmove` 를 `preventDefault` 한다 — `app`(div)에 걸어야 non-passive 다. 줄에 `user-select:none` 이 없으면 400ms 동안 안드로이드가 글자를 선택해 끌기가 시작도 못 한다.<br>★ 놓은 뒤 따라오는 `click` **하나를 삼킨다**(`catDragged`) — 안 삼키면 손을 떼는 순간 편집기가 같이 열린다 |
...
```js
{
  ...
  drag: null,          // 순서 끄는 중 { sel, from, to, lo, hi, y0, dy, rowH, on, timer, drop }
...
- [카테고리·메모 포함] 토글은 개인정보 장치이고 **기본이 꺼짐**입니다(`blankExp(false,…)`).
  이미지는 남에게 보내는 것이라, 켜짐이 기본이면 한 번의 부주의로 사적인 메모가 나갑니다.
  끄면 `exportModel()` 이 `memo`·`cat` **키 자체를 안 만듭니다** — 빈 문자열로 두면
  `row.memo != null` 검사에서 새어 나갑니다. 카테고리 이름이 **같은 스위치**를 타는 이유도
  같습니다(분류 이름 자체가 사적인 경우가 있습니다). **상세 목록도 같은 `exRow()` 를
  지나므로** 끄면 상세에서도 자동으로 빠집니다(토글 두 개가 따로 놀지 않는 지점.
  selftest 가 모델 JSON 전체에 메모 문자열이 없는지까지 봅니다).
- ⚠️ 아젠다 한 줄의 높이는 **`exRowH()` 하나**가 정합니다. 예전엔 같은 식이 일간 배치·상세
  배치·그리기 **세 곳에 따로** 적혀 있었고, 둘째 줄 조건이 `memo` 하나가 아니게 되면서
  하나만 고치면 글자가 다음 줄을 밟습니다. 다시 흩뜨리지 마세요.
...
### 4. 카테고리·할 일 순서 — **규칙·코드 완료 · 폰 터치 확인만 남음**

기본 기능은 규칙·코드 모두 올라갔고 **실기기(안드로이드 크롬)에서 확인했습니다** —
카테고리 저장이 Firestore 에 실제로 쓰입니다.

**순서 바꾸기(`order` + 끌기)는 규칙까지 배포가 끝났습니다.** 2026-08-25 에 배포된
사이트(로그인 상태·손님 아님)에서 앱이 서버로부터 받아 온 값을 읽어 확인했습니다 —
카테고리 6개가 전부 `order` 0~5 를 달고 내려오고, 할 일 85개 중 8개에 `order` 가
붙어 있었습니다. 규칙이 막고 있었다면 애초에 저장될 수 없는 값들입니다.

- ⚠️ **배포된 규칙 원문을 직접 읽은 것은 아닙니다.** Firebase CLI 에 규칙 조회 명령이
  없고 `gcloud` 도 안 깔려 있으며 콘솔 딥링크는 개요 페이지로 튕깁니다(CLI 에 저장된
  토큰을 꺼내 REST 를 부르는 것은 하지 않았습니다). **저장된 데이터로 역추적한 근거**입니다.
- `todos` 는 규칙에 필드 제약이 아예 없어서(`match /todos` 는 `allow read, write` 뿐)
  `order` 를 새로 실어도 **배포가 필요 없습니다.** categories·goals 와 다른 점입니다.

**폰 실기기 확인은 아직입니다** — 브라우저 창이 안 떠서 터치 제스처를 여기서 잴 수
없었고, 잰 것은 합성 pointer 이벤트로 도는 로직뿐입니다. 특히 **끌 때 화면이 같이
굴러가지 않는지**(손잡이의 `touch-action:none` 포함)는 기기에서 봐야 압니다. 뜨는
애니메이션도 탭이 `hidden` 이라 시계가 0 에 멈춰서, `getAnimations()` 로 160ms 를
**직접 돌려** 끝값(scale 1.03 · 그림자 · 반경 10px)만 확인했습니다 — 실제로 부드럽게
도는지는 화면에서 봐야 합니다.
