...
**하단 캡슐 탭 바(년/월/주/일)**·**첫 화면 설정**. 남은 것은 §6 의 **기간 일정 / PWA /
스토어** 셋입니다 (이미지 공유는 실기기 확인 완료, 옛 `pri` 는 버리기로 결정).
`?selftest` 전부 통과 — 개수는 `ok()` 가 세어 콘솔에 찍습니다.
**숫자를 여기 적지 마세요**(적는 순간 낡습니다).

> ⚠️ **연간 뷰·하단 바는 아직 실기기 확인 전입니다.** 데스크톱 브라우저를 360·375px 로
> 좁혀 잰 값뿐이라 폰의 근거가 아닙니다(dpr·안전영역·주소창 높이가 다릅니다).
> 브라우저 창을 띄울 수 없어 스크린샷도 못 찍었습니다 — 눈으로 볼 사람이 필요합니다.
...
| `sortItems` / **`itemsOn`** | 하루종일→시간→**제목**순 정렬(`localeCompare` 를 안 쓴다 — selftest 가 `lang='en'` 에서 정렬 불변을 본다). **`itemsOn` 이 `state.filter` 를 직접 읽는다** — 화면에 할 일을 꺼내는 통로가 이 함수뿐이라(월·주·일·하단 목록·내보내기 3곳) 여기 한 줄로 전부 따라온다. 인자로 넘기면 6곳 중 하나만 빠뜨려도 그 화면에서 필터가 **조용히** 안 먹는다(내보내기가 제일 위험). 순수함을 잃는 대가로 그 실수를 없앴다 |
...
| **`render`** | **`#app` 전체 innerHTML 재생성.** 격자는 접는다 — 월간 **3**개·주간 **5**개까지만 그리고 나머지는 `cell.more`(`+N개`). 주간 5의 근거(실측): 칸 `min-height:320` 에 헤더 64·항목 33·`gap:4`·`+N개` 12 라 넓은 화면은 5개=273(여유 47)·6개=310(여유 10뿐)·7개=347(초과)이고, **390px 에서는 열이 51px 라 시간 라벨이 두 줄로 접혀 항목이 48 → 5개라도 348** 이다(접기 전 17개는 713). **항목 높이·글꼴·시간 라벨 형식을 바꾸면 5를 다시 재야 한다.** 그래서 **격자 칸은 `pill.timeLabel`(시작만), 넓은 곳은 `pill.range`(시작–종료)** 로 갈라 둔다 — 51px 칸에 범위를 넣으면 3~4줄이 된다. 월간 셀은 애초에 시간을 안 그린다(제목만). `+N개` 는 월간처럼 **그 날을 고를 뿐**(뷰 전환 아님)이고, 주간은 부모에 `data-day` 가 없어 **`+N개` 요소에 직접** 달아 준다. 하단 리스트는 안 접는다. **연간 뷰는 이 셋과 다르다** — 목표 목록 + 12칸 요약만 그리고 **일일 할 일은 한 개도 안 넣는다**(하단 리스트도 건너뛴다). 12칸은 `[data-ym]` 로 그 달 **월간 화면**으로 간다. 년/월/주/일 탭은 헤더가 아니라 **화면 아래 떠 있는 `.tabbar` 캡슐**이고, `+` 는 그 **밖**의 원이다(연간에선 `goalNew`, 나머지는 `open`). 내보내기 버튼은 연간에서 안 그린다 — `drawExport` 는 month/week/day 세 갈래뿐이라 `'year'` 가 들어가면 **주간 격자를 조용히 그린다** |
...
### ⚠️ 하단 탭 바 `.tabbar` 의 실측 근거 — 주간 뷰 "5개"와 같은 성격

`.tabbar .seg{padding:0 12px; min-width:44px}` 는 잰 값입니다. **360px**(갤럭시 다수)에서
캡슐이 쓸 수 있는 폭은 `360 − 16×2`(래퍼 패딩) `− 12`(간격) `− 50`(`+` 원) **= 266** 이고,

| | 필요 폭 | |
|---|---|---|
| en · padding 16 | 277 | **11px 초과** — `Day` 가 잘려 캡슐 안에서 스크롤됨 |
| en · padding 14 | 261 | 여유 5px — 폰트가 폴백으로 바뀌면 다시 넘침 |
| **en · padding 12** | **245** | **여유 21px** ← 채택 |
| en · padding 12 · serif 폴백 | 255 | 여유 11px |
| ko · padding 12 | 193 | 1글자 라벨이라 `min-width:44` 가 폭을 정함 |

ko 는 라벨이 1글자라 padding 만으로는 36px 뿐입니다 — `min-width:44` 로 iOS 최소 터치
타깃을 지킵니다. **라벨·글꼴·글자 크기·`+` 버튼 크기를 바꾸면 다시 재세요.**

바는 `position:fixed` 라 **레이아웃 높이를 안 먹습니다** — 그래서 주간 "5개" 산식은
그대로입니다. 본문과의 간섭은 페이지 `padding-bottom:130px` 이 막고, 끝까지 내렸을 때
마지막 카드 바닥과 바 천장 사이가 **56px**(월·주·일 모두 실측) 남습니다.

⚠️ 이 표는 **데스크톱 브라우저를 좁힌 값**입니다. 실기기 확인은 아직입니다.
...
### 6. 기간 일정 (날짜를 잇는 선) — **다음 작업**

월간 달력에 여러 날에 걸치는 일정을 선으로 그립니다. **반복도 필요**합니다(사용자 요청).
목표와 달리 이건 **달력에 그려야** 하므로 `occursOn()` 의 "한 항목 = 한 날" 전제를
정면으로 건드립니다 — 목표를 별도 컬렉션으로 뺀 것과 같은 판단을 여기서도 먼저 할 것.
