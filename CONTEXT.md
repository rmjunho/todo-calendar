...
> ⚠️ **연간 뷰·하단 바·기간 일정은 아직 실기기 확인 전입니다.** 데스크톱 브라우저를
> 360·375px 로 좁혀 잰 값뿐이라 폰의 근거가 아닙니다(dpr·안전영역·주소창 높이가 다릅니다).
...
- **아직 안 한 것:** 실기기 화면 확인 · 위 5번과 같은 상태입니다.
- **볼 것:** ① 막대가 겹친 주에서 칸 높이가 폰에서 얼마나 커지는가(층 상한 3)
  ② 종료 날짜 `type="date"` 가 폰에서 네이티브 피커를 여는가
  ③ 360px 에서 막대 안 제목이 몇 글자나 보이는가(칸 46.6px — 실측).
