...
**카테고리(색 팔레트 10종)와 카테고리 필터**. 남은 것은 §6 (PWA / 스토어)·**이미지 공유의
실기기 검증**·**옛 `pri` 값 이관**·**규칙 배포**입니다. `?selftest` 전부 통과 — 개수는
`ok()` 가 세어 콘솔에 찍습니다. **숫자를 여기 적지 마세요**(적는 순간 낡습니다).
...
| **`sheetBusy`** | **시트가 열려 있나.** 원격 스냅샷 렌더를 미룰지 판정 (`showForm \|\| exp \|\| jump \|\| showCats`). 설정·회원 관리는 여기 없다 — 입력칸이 없어서 다시 그려도 잃을 게 없다 |
...
- ⚠️ **`LEGAL.version` 이 아직 `"1.0"` 입니다.** 처리방침 본문은 그 뒤로 여러 번
  ...
  우선순위→카테고리 교체와 카테고리 이름·색 추가**). 지금은 동의
...
### 4. 카테고리 — **코드 완료 · 규칙 배포와 옛 `pri` 이관만 남음**

⚠️ **`firestore.rules` 를 아직 안 올렸습니다.** 권한을 *추가*만 하는 변경이라 규칙이
먼저입니다(§0) — 안 올리면 카테고리 저장·삭제가 전부 거부됩니다.
...
때 "할 일이 다 사라졌다"로 읽힙니다. 그래서 규칙의 `validSettings()` 도 안 건드렸습니다.
