...
**옛 인쇄 달력 격자(§디자인 5 — 화면과 내보내기 이미지 양쪽)**.
...
4. **`.card` 를 쓰는 곳은 9군데**입니다(그중 월/주/일 뷰와 연간 12칸은 `.cal-sheet` 가
   모서리·테두리를 덮어씁니다 — §5): 로그인 카드, 월/주/일 뷰, 할 일 리스트,
   ...
   단색이라 sheen 이 없습니다.
5. **년(12칸)·월·주·일 본문은 옛 인쇄 달력 격자입니다** (사용자가 국내 벽걸이 달력
   사진을 레퍼런스로 줬습니다). 규칙은 전부 `css/style.css` 의 `.cal-*` 에 있고
   `calendar.js` 는 클래스만 붙입니다. 헤더·필터 칩·탭바는 **그대로 iOS 질감**입니다.
   요일 머리줄 아래 선과 일간 시간선도 여기 포함이고, sheen 이 아니라 괘선입니다.
   - `--cal-rule: var(--label)` / `--cal-rule-in: label 42%`. 잉크를 `#000` 으로 박으면
     **어두운 테마에서 격자가 사라집니다** — 레퍼런스는 인쇄물이라 다크 모드가 없어서
     `label` 에 묶어 자동 반전시킨 것입니다.
   - **인쇄 잉크 3색**: `--cal-red` `#DE2E24` · `--cal-blue` `#1450A3` · `--cal-paper`
     `#FCFAF6`(어두운 테마는 각각 `#FF6B5E` · `#6FA6FF` · `var(--bg)`). iOS 색이
     아닙니다. ⚠️ **`--tint` 와 `CAT_COLORS` 는 절대 물들이지 마세요** — 앞의 것은
     버튼·탭바·링크·포커스 링이 전부 쓰고, 뒤의 것은 `firestore.rules` 의 `catColor()`
     가 같은 집합을 강제합니다(§디자인 8). 그래서 잉크를 **따로** 만든 것입니다.
   - 숫자 글꼴은 `800` + 자간 `-.5px` 입니다. 레퍼런스의 굵고 좁은 그로테스크를
     **새 웹폰트 없이** 흉내 낸 것입니다 — CDN 글꼴이 늦어 폴백으로 그려지면
     `WEEK_FIT`·`blockTier` 같은 실측값이 통째로 흔들립니다.
   - 알약·막대·일간 블록의 모서리는 `2px` 입니다(각진 격자 안에서 둥근 것만 iOS 로
     남아 따로 놀았습니다). 색은 안 건드렸습니다 — 그건 데이터가 정합니다.
   - `.cal-sheet` 가 `.card` 의 둥근 모서리를 되돌립니다(**`.card` 보다 뒤에 와야 이깁니다**).
     테두리 **2px**: 1.5px 로 뒀더니 dpr 1.25 에서 크롬이 device px 로 **내림**해 안쪽
     괘선(1px)과 똑같은 0.8px 로 나왔습니다. 인라인 `border` 는 이 클래스를 이기므로
     네 카드에서 인라인 테두리를 뺐습니다 — **되살리지 마세요.**
   - **요일은 `[한자 / 영문]` 두 줄**이고 **일부러 로케일을 안 탑니다**(`i18n.js` 의
     `DOW_HANJA`/`DOW_EN`). `dow()` 를 대신하는 게 아닙니다 — 반복 요일 선택·폼의 요일
     표시·내보내기 상세는 계속 `dow()` 로 읽는 이름을 씁니다. **격자 머리줄에서만** 이 둘입니다.
   - `.cal-day` 의 **30px 정사각·아래 여백 4** 는 `calendar.js` 의 `barSpacer` 와 짝입니다.
     `.cell` 의 `border-top` 두께도 `barSpacer` 와 **같아야** 하고(지금 1px), 한쪽만
     고치면 기간 막대 층이 주마다 어긋납니다. selftest 가 이 등식을 잽니다.
   - 세로 괘선은 `.cal-grid > *` 가 답니다. **막대 층(`[data-bars]`)에는 걸지 마세요** —
     선이 막대 위를 지나면 하나짜리 막대 여럿으로 읽힙니다.
   - 오늘은 파란 원이 아니라 **빨간 사각 도장**입니다. `export.js` 도 같은 규칙입니다
     (`exSheet`/`exRuleH`/`exRuleV`/`exRuleDouble`/`exStamp`, 괘선은 화면의 두 배 = 2·4px).
     `exStamp` 은 도장 폭을 **`measureText` 로 재서** 잡습니다 — 40px 고정으로 뒀더니
     두 자리 날짜가 도장 밖으로 삐져나왔습니다.
   - ⚠️ **캔버스 머리줄 높이(`M_DOW` 56 · `W_DOW` 96)는 못 키웁니다.** selftest 가
     내보내기 이미지 크기(`1150`/`1326`/`598`/`1700`)를 못 박아 뒀고, 그 근거가
     "예전에 내보낸 그림과 나란히 놨을 때 어긋나면 안 된다" 입니다. 그래서 요일 두 줄과
...
7. **7열 격자는 `repeat(7, minmax(0,1fr))`.** `1fr`(=`minmax(auto,1fr)`)로 쓰면 nowrap
   ...
   지금은 두 뷰가 같은 `.cal-grid` / `.cal-head` 를 쓰므로 이 값이 **한 곳**에 있습니다.
