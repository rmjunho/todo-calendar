...
| **`subLine` / `syncMemoBtns`** | 아래 목록의 **둘째 줄** = [카테고리(그 색)] · [메모] [펼침]. 카테고리가 없는 항목엔 이름을 안 붙인다(폴백이 '없음' 이라 잡음이 된다).<br>★ 펼침 버튼은 **메모가 실제로 잘린 줄에만** 뜬다. 잘렸는지는 CSS 로 못 묻고 `scrollWidth > clientWidth` 로 **그린 뒤에** 재야 해서, `render()` 끝(innerHTML **뒤**)에서 `syncMemoBtns()` 가 켜고 끈다. markup 은 늘 `hidden` 으로 낸다 — 반대면 짧은 메모에서도 버튼이 한 번 번쩍인다.<br>★ 펼친 표시는 **`state.memoOpen`** 에 있다. `render()` 가 `#app` 을 통째로 다시 만들어서 요소에 두면 다음 렌더에 조용히 접힌다. 저장되지 않는 화면 전용 값이다.<br>★ 클릭 위임에서 `[data-memo]` 는 **`[data-open]` 앞**이다. 버튼이 그 안에 들어 있어서, 뒤에 두면 펼치려 누를 때마다 편집기가 열린다.<br>★ 메모 칸은 `flex:1;min-width:0` 이라야 `.trunc` 가 먹는다 — 없으면 말줄임 대신 줄이 가로로 밀린다 |
