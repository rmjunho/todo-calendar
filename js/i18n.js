...
// 달력 머리줄 전용. 옛 인쇄 달력이 요일을 [한자 / 영문] 두 줄로 찍는 것을 그대로
// 따른 것이라 **일부러 로케일을 안 탄다** — 한글 화면이든 영문 화면이든 같은 판이다.
// ★ dow() 를 대신하는 것이 아니다. 반복 요일 선택·폼의 요일 표시·내보내기 상세는
//   계속 dow() 를 쓴다(거기서는 읽는 이름이라야 한다). 이 둘은 **격자 머리줄에서만**.
