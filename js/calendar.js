...
// 실측(**페이퍼로지**, document.fonts.ready 이후 canvas measureText):
//   '오후 12:30 – 오후 12:30' 11px/500 = 118.70  ← ko 가 최장 (en 은 117.14)
//   '가나…'                  13px/600 =  33.89
//   T_FULL  = ceil(118.70) + 21 = 140   시간 범위가 **잘리지 않는** 최소 폭
//   T_TITLE = ceil(33.89)  + 21 =  55   이 아래는 '…' 만 남아 정보가 0
//   ※ Pretendard 때는 110.05 / 33.11 이라 T_FULL 이 132 였다. 글꼴을 바꾸면서
//     시간 라벨이 8px 넓어져 임계값도 같이 올라간 것이다.
// ★ 글꼴·글자 크기·패딩·구분선을 바꾸면 다시 잴 것 — 주간 뷰의 WEEK_FIT 과 같은 성격이다.
//   열 폭은 (vw-16×2-16×2-58-6)/cols 다. 실기기 폭별 2열 결과:
//     360(갤럭시 다수) 116 · 384 128 · 390 131 → title (140 에 못 미친다)
//     412 142 · 1024 448 → full (142 >= 140)
//   ※ 임계값이 132→140 으로 오르면서 **392~408px 사이의 2열만** full 에서 title 로
//     내려갔다. 그 폭에서는 시간 라벨이 잘렸을 것이므로 내려가는 쪽이 맞다.
